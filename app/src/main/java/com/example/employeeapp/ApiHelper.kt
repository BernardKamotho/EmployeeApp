...
class ApiHelper(var context: Context) {
    ...
    fun get_login(api: String, jsonData:JSONObject, callBack: CallBack) {
        val client = AsyncHttpClient(true, 80, 443)
        //GET to API
        val jsonData = StringEntity(jsonData.toString())
        client.get(context, api, jsonData, "application/json",
            object : JsonHttpResponseHandler(){
                override fun onSuccess(
                    statusCode: Int,
                    headers: Array<out Header>?,
